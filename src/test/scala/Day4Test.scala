import Day4.{checkPassports, checkPassportsAndFields, transformInput}
...
class Day4Test extends FlatSpec {
  ...
  it should "find valid passports" in {
    ...
    assert(checkPassportsAndFields(transformInput(input)).equals(4))
  ...
  it should "do the thing" in {
    ...
    println(checkPassports(transformInput(input)))
    println(checkPassportsAndFields(transformInput(input)))
  }
